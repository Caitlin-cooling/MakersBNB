...
task :setup do
  ['makersbnb', 'makersbnb_test'].each do |database|
    ...
    connection.exec('CREATE TABLE bookings(id SERIAL PRIMARY KEY, posting_id INTEGER REFERENCES postings(id) ON DELETE CASCADE, owner_id INTEGER REFERENCES users(id) ON DELETE CASCADE, user_id INTEGER REFERENCES users(id) ON DELETE CASCADE, status VARCHAR(60));')
  end
end
