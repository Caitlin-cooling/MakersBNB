...
class Posting
  ...
  def self.all
    result = DatabaseConnection.query('SELECT * FROM postings ORDER BY ' \
      'date DESC;')
    result.map do |post|
      Posting.new(id: post['id'], title: post['title'],
                  description: post['description'], price: post['price'],
                  date: post['date'], user_id: post['user_id'],
                  date_from: post['date_from'], date_to: post['date_to'])
    end
  end
